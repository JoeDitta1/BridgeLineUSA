  ...
  try {
    ...
    if (!qbSettings) {
      ...
      qbSettings = {
        ...
        connection_mode: 'single_user',
        is_sandbox: 1,
        search_item_types: 'inventory,non_inventory,service',
        search_fields: 'name,description,manufacturer_part_number',
...
router.put('/qb-settings', (req, res) => {
  try {
    ...
    // Prepare update statement with all possible fields
    const upsertStmt = db.prepare(`
      INSERT OR REPLACE INTO qb_settings (
        id, enabled, connection_type, company_file_path, qb_app_path, connection_mode,
        username, password, company_id, client_id, client_secret, access_token,
        is_sandbox, search_item_types, search_fields, preferred_vendors,
        connection_timeout, search_limit, updated_at
      ) VALUES (
        1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now')
      )
    `);
