...
async function findFileAnySubdir(customerName, quoteNo, filename) {
  ...
  // helper: recursively search a directory for a file with matching basename
  const findRecursive = (baseDir, target) => {
    if (!fs.existsSync(baseDir)) return null;
    const entries = fs.readdirSync(baseDir, { withFileTypes: true });
    for (const ent of entries) {
      const p = path.join(baseDir, ent.name);
      if (ent.isFile()) {
        if (ent.name === target) return p;
      } else if (ent.isDirectory()) {
        const found = findRecursive(p, target);
  ...
  for (const s of subdirs) {
    ...
    const foundPath = findRecursive(base, filename);
    if (foundPath) {
      return { fullPath: foundPath, subdir: s };
    }
    // fallback to shell find which can be more reliable in some envs
    try {
      const args = [base, '-type', 'f', '-name', filename, '-print', '-quit'];
      console.debug('[find fallback] execFileSync find', { base, filename, args });
      const out = String(execFileSync('find', args, { encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] } || '')).trim();
      console.debug('[find fallback] out', out);
      if (out) return { fullPath: out, subdir: s };
    } catch (e) {
      console.debug('[find fallback] error', e && e.message ? e.message : e);
    }
  }
  return null;
}
...
router.delete('/:quoteNo/files/:filename', async (req, res) => {
  try {
  const { quoteNo, filename } = req.params;
  // allow client to pass customer to avoid relying solely on DB lookup
  const customerName = String(req.query.customer || req.body?.customerName || getCustomerByQuoteNo(quoteNo) || '').trim();
...
router.delete('/:quoteNo/files/:subdir/:filename', async (req, res) => {
  ...
  const customerName = String(req.query.customer || req.body?.customerName || getCustomerByQuoteNo(quoteNo) || '').trim();
  if (!customerName) return res.status(404).json({ ok: false });
  console.debug('[delete subdir] start', { customerName, quoteNo, subdir, filename });
  const qdir = path.join(getQuoteDir(customerName, quoteNo), safeFolderName(subdir));
  const p = path.join(qdir, filename);
  console.debug('[delete subdir] directPath', p);
  if (fs.existsSync(p) && fs.statSync(p).isFile()) {
    await fsp.unlink(p);
    console.debug('[delete subdir] removed direct', p);
    return res.json({ ok: true });
  }
  // fallback: try to find under nested uuid/original directories
  const found = await findFileAnySubdir(customerName, quoteNo, filename);
  console.debug('[delete subdir] findFileAnySubdir result', found);
  if (found) {
    await fsp.unlink(found.fullPath);
    console.debug('[delete subdir] removed nested', found.fullPath);
    return res.json({ ok: true, removed: found.fullPath });
  }
  console.debug('[delete subdir] not found');
