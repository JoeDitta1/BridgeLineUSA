...
function sendSSE(res, data) {
  try {
    ...
    console.log('SSE write failed:', err.message);
    return false;
  }
}
...
router.get('/stream', (req, res) => {
  ...
  sendSSE(res, { status: 'connected', message: 'Connected to backup stream', timestamp: Date.now() });
  ...
  const heartbeatInterval = setInterval(() => {
    ...
    try {
      ...
      console.log('Heartbeat write failed, connection likely closed:', err.message);
      clearInterval(heartbeatInterval);
    }
  }, 10000); // Send heartbeat every 10 seconds (reduced from 15)
