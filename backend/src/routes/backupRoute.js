...
// POST /api/backups/switch-to-dev - Switch back to dev branch
router.post('/switch-to-dev', (req, res) => {
  try {
    const process = spawn('git', ['checkout', 'dev'], {
      stdio: ['ignore', 'pipe', 'pipe'],
      cwd: '/workspaces/BridgeLineUSA'
    });
    ...
    process.on('close', (code) => {
      if (code === 0) {
        res.json({ 
          ok: true, 
          message: 'Successfully switched to dev branch',
          output: output.trim()
        });
      } else {
        res.status(500).json({ 
          ok: false, 
          error: 'Failed to switch to dev branch',
    ...
    console.error('[backup] Error switching to dev:', error);
    res.status(500).json({ ok: false, error: error.message });
  }
});
