...
export default function QuoteFolderView() {
  ...
  // derived: keys for all files currently shown
  const allKeys = files.map(f => {
    const filename = f.label || f.name || f.filename || f.base || (f.path || "").split("/").pop();
    return `${f.subdir || ''}::${filename}`;
  });
  const allSelected = files.length > 0 && selectedKeys.length === allKeys.length;
    ...
    <div style={{ padding: 20, maxWidth: 900, margin: "0 auto", position: 'relative' }}>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        <button
          ...
          style={{ padding: '8px 10px', borderRadius: 8, background: files.length ? '#fff' : '#f3f4f6', color: '#374151', border: '1px solid #e5e7eb', cursor: files.length ? 'pointer' : 'default', marginLeft: 6 }}
        >
          {allSelected ? 'Unselect all' : `Select all (${allKeys.length})`}
        </button>
        <button
          type="button"
          onClick={async () => {
