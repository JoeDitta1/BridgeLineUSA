...
export default function QuoteLog() {
  ...
  const handleDeleteQuote = async (quoteId, quoteNo) => {
    if (!confirm(`Are you sure you want to permanently delete quote ${quoteNo}? This action cannot be undone.`)) {
      return;
    }
    try {
      const res = await fetch(`${API_BASE}/api/quotes/${quoteId}`, { method: 'DELETE' });
      const data = await res.json();
      if (!data.ok) throw new Error(data.error || 'Failed to delete');
      
      // Remove from local state
      setRows(current => current.filter(r => r.id !== quoteId));
      alert(`Quote ${quoteNo} has been deleted.`);
    } catch (e) {
      alert(`Error deleting quote: ${e.message}`);
    }
  };

  // Archive quote function
  const handleArchiveQuote = async (quoteId, quoteNo) => {
    if (!confirm(`Archive quote ${quoteNo}? This will move it to archived status and move the quote folder to _archived.`)) {
      return;
    }
    try {
      const res = await fetch(`${API_BASE}/api/quotes/${quoteId}/archive`, { method: 'POST' });
      const data = await res.json();
      ...
      setRows(current => current.map(r => 
        r.id === quoteId ? { ...r, status: 'Archived' } : r
      ));
      alert(`Quote ${quoteNo} has been archived.`);
    } catch (e) {
      alert(`Error archiving quote: ${e.message}`);
    }
  };
    ...
    {r.quote_no}
                ...
                <td style={styles.td}>
                  <div style={{ display: 'flex', gap: '4px' }}>
                    <button 
                      style={{...styles.btnSm, ...styles.archiveBtn}} 
                      onClick={() => handleArchiveQuote(r.id, r.quote_no)}
                      title="Archive quote"
                    >
                      📦
                    </button>
                    <button 
                      style={{...styles.btnSm, ...styles.deleteBtn}} 
                      onClick={() => handleDeleteQuote(r.id, r.quote_no)}
...
const styles = {
  ...
  btnSm: { padding: '6px 10px', border: '1px solid #ddd', borderRadius: 6, background: '#f8f8f8', cursor: 'pointer' },
  archiveBtn: { background: '#fff3cd', borderColor: '#ffeaa7' },
  deleteBtn: { background: '#f8d7da', borderColor: '#f5c2c7' },
