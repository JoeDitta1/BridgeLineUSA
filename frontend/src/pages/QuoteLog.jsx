          ...
          <tbody>
            {paged.map(r => (
              <tr key={r.id}>
                ...
                {/* Make quote number clickable → Customer folder page */}
                <td style={styles.td}>
  <Link to={`/quotes/customers/${encodeURIComponent(r.customer_name || '')}`}>
    {r.quote_no}
  </Link>
</td>
        ...
        <div>
          <button
            ...
            onClick={async () => {
              ...
              if (!window.confirm(`Delete ${selectedIds.length} selected quote(s)? This will remove their DB rows and (attempt to) remove their folders.`)) return;
              try {
                for (const id of selectedIds) {
                  await fetch(`${API_BASE}/api/quotes/${encodeURIComponent(id)}`, { method: 'DELETE', credentials: 'include' });
                }
                setSelectedIds([]);
                await loadQuotes();
              } catch (e) {
