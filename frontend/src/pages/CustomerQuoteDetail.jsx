      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          ...
          <button
            ...
            onClick={async () => {
              if (!window.confirm(`Delete customer "${customer}" and all quotes? This cannot be undone.`)) return;
              try {
                const res = await fetch(`/api/quotes/customers/${encodeURIComponent(customer)}`, { method: 'DELETE', credentials: 'include' });
                const js = await res.json().catch(()=>({}));
                if (!res.ok || js.ok === false) throw new Error(js.error || `HTTP ${res.status}`);
                // navigate back to customers list
                navigate('/quotes/customers');
              } catch (e) {
                alert('Failed to delete customer: ' + (e?.message || e));
              }
            }}
          >
            Delete Customer
